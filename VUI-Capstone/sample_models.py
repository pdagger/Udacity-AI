...
def rnn_model(input_dim, units, activation, output_dim=29):
    ...
    time_dense = TimeDistributed(Dense(output_dim), name='timed_bn_rnn')(bn_rnn)
    # Add softmax activation layer
    ...
    for layer in range(recur_layers):
    	...
    	if layer == 0:
    		simp_rnn = GRU(units, activation='relu', \
    			return_sequences=True, implementation=2, name='rnn_'+str(layer+1))(input_data)
    	else:
        	simp_rnn = GRU(units, activation='relu', \
        		return_sequences=True, implementation=2, name='rnn_'+str(layer+1))(bn_rnn)
    	# Add batch normalization 
    	bn_rnn = BatchNormalization(name='bn_simp_rnn_'+str(layer+1))(simp_rnn)
    # DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim), name='timed_bn_rnn')(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(GRU(units, activation='relu', \
    			return_sequences=True, implementation=2), merge_mode='concat', name='bidi_rnn')(input_data)
    # DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim), name='timed_bidi_rnn')(bidir_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    simp_rnn1 = SimpleRNN(units, activation='relu',
        return_sequences=True, implementation=2, name='rnn1')(bn_cnn)
    # Add batch normalization
    ...
    time_dense = TimeDistributed(Dense(output_dim), name='timed_bn_rnn2')(bn_rnn2)

...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # i = len(a_node.prenodes)
            # for literal in a_node.prenodes:
            #     if literal in self.s_levels[level]:
            #         i -= 1
            # if i == 0:
            #     self.a_levels[level].add(a_node)
            #     self.s_levels[level].children.add(a_node)
            # for literal in action.precond_pos:
            #     if literal == self.s_levels[level].effects:
            #         self.a_levels[level].add(PgNode_a(literal, True))
            # for literal in action.precond_neg:
            #     if literal == self.s_levels[level].effect_rem:   
            #         self.a_levels[level].add(PgNode_a(literal, False))            
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            for s_node in a_node.effnodes:
                #s_node = PgNode_s(node.symbol, node.is_pos)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #Check node_a1 effects with node_a2 preconditions
        for effect in node_a1.action.effect_add:
        ...
        #Check node_a2 effects with node_a1 preconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #Parents are preconditions of actions
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        i = max(len(node_s1.parents), len(node_s2.parents))
        ...
        for goal in self.problem.goal:
            ...
            for s_level in self.s_levels:
                for a_node in self.a_levels[level]:
                    if goal in a_node.action.effect_add:
                        level_sum += level + 1
                    break
                level += 1
                break
